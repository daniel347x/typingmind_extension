...
(function() {
  ...
  function renderPayloadCaptureModal() {
    ...
    items.forEach((cap, idx) => {
      ...
      if (sessionName) {
        bottomPartsHtml.push('<span data-action="set-session-name" data-session-id="' + escapeHtml(capSessionId || capPastedId) + '" title="Click to rename this session" style="cursor:pointer;color:' + modelColor + ';font-size:32px;font-weight:bold;">' + escapeHtml(sessionName) + '</span>');
      }
